...
- (void)playAudioWithData:(NSData *)audioData withTag:(NSInteger)tag{
    
    self.TagViewTagNum = tag;
...
- (void)setAudioPlayerState:(LGAudioPlayerState)audioPlayerState{
	...
	if (self.delegate && [self.delegate respondsToSelector:@selector(audioPlayerStateDidChanged:forIndex:)]) {
		[self.delegate audioPlayerStateDidChanged:_audioPlayerState forIndex:self.TagViewTagNum];
	}
