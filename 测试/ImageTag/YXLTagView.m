...
- (void)setIsAudio:(BOOL)isAudio{
    ...
    if (isAudio) {
        ...
        _audioViewLabel =[XHAudioImageView new];
        _audioViewLabel.image=[UIImage imageNamed:@"textTag"];
        _audioViewLabel.userInteractionEnabled=YES;
     
        _audioViewLabel.hidden= YES;
        [_audioViewLabel sizeToFit];
        [_audioViewLabel.labelTime addObserver:self forKeyPath:@"text" options:NSKeyValueObservingOptionNew context:nil];
        [self addSubview:_audioViewLabel];
        [_audioViewLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(@(8));
            make.centerY.equalTo(self);
...
-(void)setIsPositiveAndNegative:(BOOL)isPositiveAndNegative{
    ...
    if (CGWidth(imageLabelIcon)-15 > size.width) {
        ...
        W= self.imageLabel ? size.width-(CGWidth(imageLabelIcon)-15) : 55 - 5 - CGWidth(imageLabelIcon) - 15;
    }
...
//            UIImage *image =[UIImage imageNamed:@"textTagAnti"];
//            _imageLabel.image=[image resizableImageWithCapInsets:UIEdgeInsetsMake(3, 3, 3, 9)];
//            _audioViewLabel.image=[image resizableImageWithCapInsets:UIEdgeInsetsMake(3, 3, 3, 9)];
//            [_audioViewLabel mas_updateConstraints:^(MASConstraintMaker *make) {
...
//            }];
            ...
            UIImage *image =[UIImage imageNamed:@"textTag"];
            _imageLabel.image=[image resizableImageWithCapInsets:UIEdgeInsetsMake(3, 9, 3, 3)];
            _audioViewLabel.image=[image resizableImageWithCapInsets:UIEdgeInsetsMake(3, 9, 3, 3)];
            [_audioViewLabel mas_updateConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(@8);
            }];
